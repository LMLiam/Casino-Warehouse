...
Run the narrowest meaningful checks while iterating, then run the broader checks that cover the changed surface before opening or updating a pull request. For UI, browser workflow, multiplayer, or visual changes, include Playwright coverage with `npm run visual` or `npm run visual:serial`. The `Build, Test, and Visual Checks` workflow displays those lanes as `Playwright Visual and E2E (Laptop Visual)`, `Playwright Visual and E2E (Tablet Visual)`, and `Playwright Visual and E2E (Laptop Multiplayer)` with the aggregate `Required Quality Gate` check; reproduce one lane locally with `npm run visual -- --project=laptop tests/e2e/casino-visual.spec.ts`.
...
- Maintain the required CodeQL workflow status check and other required checks. The CI aggregate appears on pull requests as `Build, Test, and Visual Checks / Required Quality Gate`, and the repository ruleset uses the `Required Quality Gate` status context. The native CodeQL `code_scanning` ruleset rule is currently disabled because GitHub left its generated `CodeQL` check queued after successful analyses; re-enable it only after a pull request proves that check completes reliably. GitHub Code Quality is documented as externally blocked for this personal-account repository until eligibility changes.
