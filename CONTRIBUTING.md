...
The required pull request status contexts are `Required Quality Gate`, `Analyze (javascript-typescript)`, `PR Standards`, and `Review Dependency Changes`. GitHub displays the CI aggregate as `Build, Test, and Visual Checks / Required Quality Gate`. Its prerequisite checks use descriptive names such as `Build, Test, and Visual Checks / Quality, Coverage, and Server Build` and `Build, Test, and Visual Checks / Playwright Visual and E2E (Laptop Visual)`.
