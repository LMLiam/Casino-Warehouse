...
| Required PR checks pass on the latest pushed commit           | Kept out of this static audit so transient CI state does not go stale; check the pull request checks and final readiness report for the current head.                                              | PR evidence |
...
Final pushed commit SHA and CI status are recorded in the pull request readiness report after push rather than checked into this static audit file.
