/**
 * Renders the main table panel and its game-specific sections.
 *
 * @returns HTML for the lobby, table views, HUD, wallet, and action dock.
 */
export const renderTablePanel = (): string => {
  const gameLobbySection = (): string => `
...
`;
  ...
  <div id="tableHost" class="table-host">
    ...
    <div id="beatTableStatus" class="table-status-overlay hidden" role="status" aria-live="polite" aria-atomic="true" aria-label="Beat the House table status"></div>
  </div>
  <div id="beatSettlementAnnouncement" class="sr-only" role="status" aria-live="polite" aria-atomic="true"></div>
`;

  const blackjackTableSection = (): string => `
...
`;
  ...
  const slotsTableSection = (): string => `
...
`;
  ...
  const beatControlsPanel = (): string => `
...
`;
  ...
  const blackjackControlsPanel = (): string => `
...
`;
  ...
  const slotsControlsPanel = (): string => `
...
`;
  ...
  <details class="admin-panel hud-overflow-section" data-hud-section="admin">
    ...
    <div class="admin-menu-content">
      ...
      <label>Add bankroll <input id="moneyInput" type="number" min="1" value="1000" /></label>
      ...
      <details class="log-panel"><summary>Game Log</summary><div id="log"></div></details>
      <details class="log-panel"><summary>Audit</summary><div id="auditLog"></div></details>
      <details class="log-panel"><summary>Responsible Play</summary>
        <label>Session wager limit <input id="sessionLimitInput" type="number" min="0" value="0" /></label>
      </details>
    </div>
  </details>
`;

  const hudOverflowMenu = (): string => `
  <details id="hudOverflowMenu" class="hud-overflow-menu">
    <summary id="hudOverflowButton" aria-label="Game actions menu" title="Game actions"><span aria-hidden="true">...</span></summary>
    <div id="hudOverflowPanel" class="hud-overflow-panel" aria-label="Secondary game actions">
      ...
      ${infoPanel()}${profilePanel()}${roomPanel()}${statsPanel()}${adminPanel()}
    </div>
  </details>
`;

  const gameHudSection = (): string => `
  <div id="gameHud" class="game-hud" aria-label="Game menus">
    <div class="hud-button-row"><button id="backToLobbyBtn" class="hud-button primary" type="button">Home</button></div>
    <div class="hud-button-row hud-button-row-right">${hudOverflowMenu()}</div>
  </div>
`;

  const walletSection = (): string => `
  <div class="wallet-stack">
    <div id="moneyPill" class="money-pill" aria-label="Player balance">
      <span>Balance</span>
      <div class="bankroll-line"><strong id="bankroll">£1,000</strong><em id="bankrollDelta" aria-live="polite"></em></div>
      <small id="houseAdvancePill" class="hidden"></small>
    </div>
    <div id="beatHalfChipIndicator" class="beat-half-chip-indicator hidden" role="status" aria-live="polite" aria-label="Beat the House half-chip balance">Half chip: one half</div>
  </div>
`;

  return `
  <section class="table-panel" aria-label="Casino game area">
    ${gameLobbySection()}${roomLobbySection()}${beatTableSection()}${blackjackTableSection()}${slotsTableSection()}
    ${gameHudSection()}${walletSection()}${actionDockSection()}
  </section>
`;
};
